...
{$IFDEF WINDOWS}
procedure libc_free(p: Pointer); cdecl; external 'msvcrt.dll' name 'free';
{$ELSE}
{$IFDEF LINUX}
procedure libc_free(p: Pointer); cdecl; external 'libc.so' name 'free';
{$ELSE}
{$IFDEF DARWIN}
procedure libc_free(p: Pointer); cdecl; external 'libSystem.dylib' name 'free';
{$ELSE}
procedure libc_free(p: Pointer); cdecl; external; // Заглушка для других платформ
{$ENDIF}
{$ENDIF}
{$ENDIF}
...
begin
  ...
  Result := (@gpa_init_log <> nil) and (@gpa_close_log <> nil) and
            ...
            (@gpa_archive_changed <> nil) and (@gpa_get_files_in_directory <> nil);
